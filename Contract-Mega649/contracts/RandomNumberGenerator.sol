...
interface IRandomNumberGenerator {
     ...
     * Views random result
     */
    function viewRandomResult() external view returns (uint[6] memory);
}
...
contract RandomNumberGenerator is VRFConsumerBase, IRandomNumberGenerator, Ownable {
     ...
     * @param _seed: seed provided by the JpmSwap lottery
     */
    function getRandomNumber(uint256 _seed) external override {
        require(msg.sender == MegaLottery649, "Only MegaLottery649");
    ...
    /**
     * @notice Set the address for the MegaLottery649
     * @param _MegaLottery649: address of the JpmSwap lottery
     */
    function setLotteryAddress(address _MegaLottery649) external onlyOwner {
        MegaLottery649 = _MegaLottery649;
    }
     ...
     */
    function viewRandomResult() external view override returns (uint256[6] memory) {
        return drawnNumbers;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        randomResult = uint32(333333 + randomness);
    ...
    function chooseNumbers() internal{

        while(checkDuplicate(drawnNumbers) == true){
                for (uint256 i = 0; i < 6; i++) {
                    drawnNumbers[i] = expand(uint256(randomResult), 6)[i];
                }
                randomResult = randomResult +333333;
            }
        uint[6] memory temp = sort(drawnNumbers);
        drawnNumbers = temp;
        latestLotteryId = IMegaLottery649(MegaLottery649).viewCurrentLotteryId();
    }

    function sort(uint[6] memory data) internal pure returns (uint[6] memory) {
        quickSort(data, int(0), int(data.length - 1));
        return data;
    }

    function quickSort(uint256[6] memory arr, int left, int right) internal pure {
        int i = left;
        int j = right;
        if (i == j) return;
    ...
    function checkDuplicate(uint256[6] memory arr) internal pure returns(bool) {
