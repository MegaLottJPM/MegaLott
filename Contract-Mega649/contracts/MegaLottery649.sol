...
interface IRandomNumberGenerator {
     ...
     * Views random result
     */
    function viewRandomResult() external view returns (uint256[6] memory);
}

// File: contracts/interfaces/IMegaLottery649.sol

pragma solidity ^0.8.4;

interface IMegaLottery649 {
    /**
     ...
     * @dev Callable by operator
     */
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, bool _autoInjection,uint[] memory numberWinPerBracket, uint256[][] memory bracketWiner) external;
    ...
    // function viewDataWinForLotteryId() external returns(uint256);
}

// File: contracts/MegaLottery649.sol
...
contract MegaLottery649 is ReentrancyGuard, IMegaLottery649, Ownable {
    ...
    struct Lottery {
        ...
        uint256[4] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[4] jpmPerBracket;
        uint256[4] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInJpm;
        uint256[6] finalNumber;
    }
    ...
    //3,4,5,6
    mapping(uint256 => mapping(uint256 => string[])) public  _listNumberWinPerLotteryId;
    // Mapping are cheaper than arrays
    mapping(uint256 => Lottery) private _lotteries;
    mapping(uint256 => Ticket) private _tickets;

    // Keeps track of number of ticket per unique combination for each lotteryId
    mapping(uint256 => mapping(string => uint256)) public _numberTicketsPerLotteryId;

    // Keep track of user ticket ids for a given lotteryId
    mapping(address => mapping(uint256 => uint256[])) private _userTicketIdsPerLotteryId;

    mapping(address => mapping(uint256 => bool)) private _claimReward;
    ...
    event LotteryNumberDrawn(uint256 indexed lotteryId, uint256[6] finalNumber, uint256 countWinningTickets);
    ...
    event ClaimAll(address indexed claimer, uint256 amount);
    /**
     * @notice Constructor
        ...
        nonReentrant
    {
        ...
        jpmToken.safeTransferFrom(address(msg.sender), address(this), amountCakeToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInJpm += amountCakeToTransfer;

        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            uint256[] memory thisTicketNumber = _ticketNumbers[i];

             require((thisTicketNumber[0] < 50) && (thisTicketNumber[0] > 0) && (thisTicketNumber[1] < 50) && (thisTicketNumber[1] > 0) && 
             (thisTicketNumber[2] < 50) && (thisTicketNumber[2] > 0) && (thisTicketNumber[3] < 50) && (thisTicketNumber[3] > 0) && 
             (thisTicketNumber[4] < 50) && (thisTicketNumber[4] > 0) && (thisTicketNumber[5] < 50) && (thisTicketNumber[5] > 0) &&
             checkDuplicate(thisTicketNumber)==false && checkIncrement(thisTicketNumber) == true
            , "You can pick 6 numbers increment from 1 to 49.");
    ...
    function checkDuplicate(uint256[] memory arr) internal pure returns(bool) {
    ...
    function checkIncrement(uint256[] memory arr) internal pure returns(bool) {
        for( uint256 i = 0 ; i< arr.length-1; i++){
            for (uint256 j = i+1; j<arr.length; j++){
                if(arr[i] >= arr[j]){
                    return false;
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            require(_brackets[i] < 4, "Bracket out of range"); // Must be between 0 and 4
        ...
        jpmToken.safeTransfer(msg.sender, rewardInCakeToTransfer);
        _claimReward[msg.sender][_lotteryId] = true;

        emit TicketsClaim(msg.sender, rewardInCakeToTransfer, _lotteryId, _ticketIds.length);
    }

    function claimAll() external override notContract nonReentrant {
        require(currentLotteryId >=1, "No have round");
        uint rewardToTransfer;
        for(uint i = 1; i<=currentLotteryId;i++){
            if(_lotteries[i].status != Status.Claimable || _claimReward[msg.sender][i]==true ){
                 continue;
            }
            for(uint j = 0;j<4;j++){
                uint[] memory  ids = _idTicketWinPerLotteryId[i][j];
                for(uint k =0; k<ids.length;k++){
                    if(msg.sender == _tickets[ids[k]].owner){
                        uint amount = _lotteries[i].jpmPerBracket[j];
                        rewardToTransfer +=amount;
                    }
                }
            }
            _claimReward[msg.sender][i]=true;
        }

        // Transfer money to msg.sender
        require(rewardToTransfer >0, "No have reward");
        jpmToken.safeTransfer(msg.sender, rewardToTransfer);

        emit ClaimAll(msg.sender, rewardToTransfer);
    }
    /**
     * @notice Close lottery
     * @param _lotteryId: lottery id
    ...
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, bool _autoInjection, uint[] memory numberWinPerBracket, uint256[][] memory bracketWiner)
        external
        override
        onlyOperator
        nonReentrant
    {
        ...
        uint256[6] memory finalNumber = randomGenerator.viewRandomResult();
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInJpm) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256  amountToWithdrawToTreasury;
        // temp memory datatemp;
        // Calculate prizes in JPM for each bracket by starting from the highest one
        for (uint32 i = 0; i < 4; i++) {
            _lotteries[_lotteryId].countWinnersPerBracket[i] = numberWinPerBracket[i];
            numberAddressesInPreviousBracket += numberWinPerBracket[i];
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInJpm - amountToShareToWinners);
        ...
        if (_autoInjection) {
            ...
            jpmToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            jpmPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0)],
            ...
            finalNumber: [uint256(0), uint256(0), uint256(0), uint256(0),uint256(0), uint256(0)]
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(jpmToken), "Cannot be Lotto token");
    ...
    /**
     * @notice Set JPM price ticket upper/lower limit
     * @dev Only callable by owner
        ...
        )
    {
        ...
        DataWinerLottery[] memory dataWiner = new DataWinerLottery[](4);
       for(uint i = 0;i<4;i++){
    ...
    function _calculateRewardsForTicketId(
        ...
        // uint256[6] memory userNumber = _lotteries[_lotteryId].finalNumber;
        bool flagResult = false;
        uint[]  memory ls = _idTicketWinPerLotteryId[_lotteryId][_bracket];
