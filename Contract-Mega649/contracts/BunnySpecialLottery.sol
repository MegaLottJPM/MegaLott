...
// File: lottery/contracts/interfaces/IMegaLottery649.sol
...
interface IMegaLottery649 {
     ...
     * @param _priceTicketInJpm: price of a ticket in CAKE
...
// File: contracts/JpmBunnies.sol

pragma solidity ^0.6.0;

/** @title JpmBunnies.
 * @notice It is the contracts for JpmSwap NFTs.
 */
contract JpmBunnies is ERC721, Ownable {
    ...
    constructor(string memory _baseURI) public ERC721("Jpm Bunnies", "PB") {
 ...
 * Jpm Collectibles/Bunnies.
 */
contract BunnyMintingStation is AccessControl {
    JpmBunnies public jpmBunnies;
    ...
    constructor(JpmBunnies _jpmBunnies) public {
        jpmBunnies = _jpmBunnies;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @notice Mint NFTs from the JpmBunnies contract.
     * Users can specify what bunnyId they want to mint. Users can claim once.
     * There is a limit on how many are distributed. It requires JPM balance to be > 0.
     */
    function mintCollectible(
        ...
        uint256 tokenId = jpmBunnies.mint(_tokenReceiver, _tokenURI, _bunnyId);
        return tokenId;
    }
    ...
    function setBunnyName(uint8 _bunnyId, string calldata _bunnyName) external onlyOwner {
        jpmBunnies.setBunnyName(_bunnyId, _bunnyName);
    }
    ...
    function changeOwnershipNFTContract(address _newOwner) external onlyOwner {
        jpmBunnies.transferOwnership(_newOwner);
    }
}
 ...
 */
contract JpmProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public lottoToken;
    ...
    uint256 public numberJpmToReactivate;
    uint256 public numberJpmToRegister;
    uint256 public numberJpmToUpdate;
    ...
    constructor(
        IBEP20 _lottoToken,
        uint256 _numberJpmToReactivate,
        uint256 _numberJpmToRegister,
        uint256 _numberJpmToUpdate
    ) public {
        lottoToken = _lottoToken;
        numberJpmToReactivate = _numberJpmToReactivate;
        numberJpmToRegister = _numberJpmToRegister;
        numberJpmToUpdate = _numberJpmToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends JPM to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        lottoToken.safeTransferFrom(_msgSender(), address(this), numberJpmToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer JPM token to this address
        lottoToken.safeTransferFrom(_msgSender(), address(this), numberJpmToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        lottoToken.safeTransferFrom(_msgSender(), address(this), numberJpmToReactivate);
    ...
    /**
     * @dev Claim JPM to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        lottoToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of JPM to register
     * Callable only by owner admins.
     */
    function updateNumberJpm(
        uint256 _newNumberJpmToReactivate,
        uint256 _newNumberJpmToRegister,
        uint256 _newNumberJpmToUpdate
    ) external onlyOwner {
        numberJpmToReactivate = _newNumberJpmToReactivate;
        numberJpmToRegister = _newNumberJpmToRegister;
        numberJpmToUpdate = _newNumberJpmToUpdate;
    }
...
contract BunnySpecialLottery is Ownable {
    ...
    IMegaLottery649 public MegaLottery649;
    BunnyMintingStation public bunnyMintingStation;
    JpmProfile public jpmProfile;
    ...
    constructor(
        address _MegaLottery649Address,
        address _bunnyMintingStationAddress,
        address _jpmProfileAddress,
        uint256 _endBlock,
        ...
        MegaLottery649 = IMegaLottery649(_MegaLottery649Address);
        bunnyMintingStation = BunnyMintingStation(_bunnyMintingStationAddress);
        jpmProfile = JpmProfile(_jpmProfileAddress);
    ...
    function mintNFT(
        ...
        // Increase point on JpmSwap profile, for a given campaignId.
        jpmProfile.increaseUserPoints(msg.sender, numberPoints[_bunnyId], campaignIds[_bunnyId]);
    ...
    /**
     * @notice Change the campaignId for JpmSwap Profile.
     * @dev Only callable by owner.
     */
    ...
    /**
     * @notice Change the number of points for JpmSwap Profile.
     * @dev Only callable by owner.
     */
    ...
    function _canClaim(
        ...
        if (
            ...
            !jpmProfile.getUserStatus(_userAddress) ||
        ...
        if (_bunnyId == nftId1) {
            uint256 size;
            (, , , size) = MegaLottery649.viewUserInfoForLotteryId(_userAddress, _lotteryId, 0, 1);
            return size > 0;
        }
        if (_bunnyId == nftId2) {
            ...
            (, , ticketStatuses, size) = MegaLottery649.viewUserInfoForLotteryId(
                _userAddress,
                _lotteryId,
                _cursor,
                1
